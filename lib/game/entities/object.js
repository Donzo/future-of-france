...
EntityObject = ig.Entity.extend({
	...
	handleObjBox: function(prompt, objName, longResp, computer, conditionalResponse, coin){
		if (ig.game.clickDelayTimer.delta() > 0){
			if (prompt && !ig.game.objBoxOpen){
				...
				if (!ig.game.promptBoxOpen){
					...
					if (coin){
						ig.game.spawnEntity( EntityConfirm, 0, 0, { name: "yes", cType: "inspectPromptCoin", oName: objName });	
						ig.game.spawnEntity( EntityConfirm, 0, 0, { name: "no", cType: "inspectPromptCoin", oName: objName  });	
					}
					else if (longResp){
			...
			if (ig.input.released('action') && ig.game.clickDelayTimer.delta() > 0 && !ig.game.computerOn || ig.input.released('click') && ig.game.clickDelayTimer.delta() > 0 && !ig.game.computerOn) {
				...
				if (ig.game.longObjTxt){
					...
					if (ig.game.objTxt[objKey].hasPrompt) {
						console.log(objKey + " has a prompt.");
						hasPrompt = true;
						ig.game.playMsgOpenSound1();
					}
					//Check if Object Has a Coin
					if (ig.game.objTxt[objKey].hasCoin) {
						console.log(objKey + " has a coin.");
						hasCoin = true;
					}
					...
					this.handleObjBox(hasPrompt, objKey, longResponse, isComputer, hasConditionalResponse, hasCoin);	
				}
