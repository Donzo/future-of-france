...
EntityPlayer = ig.Entity.extend({
	...
	teleport: function(){
		if (ig.game.getEntityByName("tpPlayer")){
			...
			console.log('calling this too...')
		}
		else{
			if (ig.game.goingToLvl == 8){
				var tpVar = "tpPlayer" + ig.game.playerNeedsTpNum;
				console.log('looking for ' + tpVar)
				if (ig.game.getEntityByName(tpVar)){
					var tpPlayer = ig.game.getEntityByName(tpVar);
					this.pos.x = tpPlayer.pos.x;
					this.pos.y = tpPlayer.pos.y;
				}
				else{
					console.log('cant find ' + tpVar);
				}
			}
			else{
				console.log('ig.game.goingToLvl = ' + ig.game.goingToLvl);
