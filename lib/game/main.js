...
.defines(function(){
	...
	MyGame = ig.Game.extend({
		...
		atTitleScreen: true,
		introOn: true, //Dont forget to turn this back on AND the music too
		pause: true,
		...
		doorOpenSound2: new ig.Sound( 'media/sounds/door-open-02.*' ),
		...
		init: function() {
			...
			ig.input.bind( ig.KEY.M, 'stopMusic' );
			...
			this.clickDelayTimer2 = new ig.Timer(0);
			...
			ig.game.spawnEntity( EntityButton, 0, 0, { name: "contact" });
		...
		playDoorOpenSound2: function(){
			if (!ig.game.muteGame){
				ig.game.doorOpenSound2.volume = .25; 
				ig.game.doorOpenSound2.play();
			}	
		},
		...
		borrowMintAndTransferGHO: function(){
			providerCheck("Ethereum Sepolia", 14);
		},
		mintBlueCoins: function(){
			providerCheck("Arbitrum Sepolia", 15);
		},
		npcConditionExample: function(){
			alert('conditional!')
		},
		...
		update: function() {
			...
			if (this.cutScreenOn){
				...
				if ( ig.input.pressed('action') || ig.input.released('click')){
					if (this.clickDelayTimer2.delta() >= 0){
						this.handleCutTxtSeq();
						this.clickDelayTimer2.set(1);	
					}
				}
			}
			...
			//Stop Music
			if (ig.input.released('stopMusic')){
				if (!this.musicStopped){
					...
					else if (this.currentLvl == 12){
						ig.game.playMusicBro(4);
					}
					else if (this.currentLvl == 12){
						ig.game.playMusicBro(2);
					}
					else if (this.currentLvl > 8){
		...
		processResponse: function(text) {
			// Remove specific undesirable symbols
			text = text.replace(/[#*-]/g, '');

			// Replace numbered list items with words
			text = text.replace(/\b1\.\s/g, 'First, ');
			text = text.replace(/\b2\.\s/g, 'Second, ');
			text = text.replace(/\b3\.\s/g, 'Third, ');
			text = text.replace(/\b4\.\s/g, 'Fourth, ');
			text = text.replace(/\b5\.\s/g, 'Fifth, ');
		...
		setCutScreenTxt: function(which){
			...
			if (which == 1){
				...
				this.cutTxt.ct1 = "The year was 2008. You were a visionary in the Web 2.0 era, pushing the boundaries of Internet technology.";
		...
		drawTitleScreen: function(){
			...
			//Contact
			//this.font.draw("contact@fofrance.com", ig.system.width - 100, ig.system.height - 20, ig.Font.ALIGN.RIGHT );
			ig.game.contactButX = ig.system.width - 160;
			ig.game.contactButY = ig.system.height - 20;
			
			var myFontFace = this.defaultFontFace;
			ctx.font = `14px '${myFontFace}', sans-serif`;
			ctx.fillStyle = "#F8F8FF";
			ctx.fillText("contact@fofrance.com", ig.game.contactButX, ig.game.contactButY );
			...
			if (this.transitionType == "fadeOut"){
				...
				if (this.transitionTimer.delta() > 1){
					...
					if (ig.game.atTitleScreen){
						...
						var delay = 1;
						if (ig.game.goingToLvl < ig.game.currentLvl || ig.game.goingToLvl == 8){
